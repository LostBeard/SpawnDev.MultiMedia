...
> **ARCHIVED 2026-04-25.** This is the original scoping doc from when SpawnDev.MultiMedia was a planned-not-yet-existing project. It lived in the SpawnDev.RTC repo (the consumer at the time) until SpawnDev.MultiMedia got its own repo and scope shifted to Riker. The unchecked test-name list near the bottom was the planned suite at design time; the actual suite has shipped under different names + organization (146+ tests in `SpawnDev.MultiMedia.Demo.Shared` as of 0.2.0-rc.1). For the current state, see the live `Docs/`, `README.md`, and `CHANGELOG.md`. This file is preserved as a "what we set out to build" historical reference.

**Owner:** Captain (TJ)
**Lead:** Geordi (originally) → Riker (current, per lane scope in `D:\users\tj\Projects\CLAUDE.md`)
**Template:** Copy SpawnDev.RTC project layout exactly (PlaywrightMultiTest, Demo, DemoConsole, Demo.Shared, etc.) — done
**Repo:** [LostBeard/SpawnDev.MultiMedia](https://github.com/LostBeard/SpawnDev.MultiMedia) — exists, shipping 0.2.0-rc.1
